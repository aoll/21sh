...
/*   Updated: 2017/03/15 16:49:19 by alex             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_cursor *cursor, t_arr *arr, int index_start_showed, int is_total)
{
	...
	{
		s_line = *(unsigned char **)(
			(unsigned char *)arr->ptr + arr->sizeof_elem * index_start_showed);
		...
		if (index_line == cursor->terminal_size.ws_col - 1 || *s_line == 10)
		{
			index_line = -1;
			nb_line++;
			ft_cursor_nb_line_set_cursor(
				cursor, arr, s_line, index_start_showed);
		}
		index_line++;
	}
	return (nb_line);
