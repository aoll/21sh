...
int  ft_builtin_cd_set_env(t_arr **envp, const char *key, char *value)
{
  int index;
  t_kval *kval;
  t_arr *env;
...
int  ft_builtin_cd_is_existing_file(
  const char *path_absolute, const char *new_path)
{
  int err;

  if ((err = access(path_absolute, F_OK)))
  {
    ft_putstr("21sh: cd no file: ");
    ft_putstr(new_path);
    ft_putstr("\n");
    return (EXIT_FAILURE);
  }
...
int  ft_builtin_cd_change_directory(
  t_arr **env, char *old_path, char *new_path)
{
  char *path_absolute;
  int err;
  char *new_actual_pwd;
  ...
  if (!(path_absolute = ft_builtin_cd_absolute_path(
  ...
  ft_builtin_cd_set_env(env, "PWD", new_actual_pwd);
  ft_builtin_cd_set_env(env, "OLDPWD", old_path);
  return (EXIT_SUCCESS);
}
 ...
 * else nothing append
 */
int  ft_builtin_cd_old_pwd(t_arr **envp, char *actual_pwd)
{
  int index_old_pwd;
  t_kval *kval;
  int err;
  ...
  {
    ft_putstr("21sh: OLDPWD not present: -\n");
    return (EXIT_FAILURE);
  }
  kval = *(t_kval **)((unsigned char *)env->ptr + index_old_pwd * env->sizeof_elem);
  if ((err = ft_builtin_cd_change_directory(envp, actual_pwd, kval->value)))
  {
    return (EXIT_FAILURE);
  }
  return (EXIT_SUCCESS);
 ...
 *
 */
static int  ft_builtin_cd_path(t_arr **env, char *path, char *actual_pwd)
{
  int err;

  if (!ft_strcmp(path, "-"))
  {
    if ((err = ft_builtin_cd_old_pwd(env, actual_pwd)))
    {
      return (EXIT_FAILURE);
    }
  }
  ...
  {
    if ((err = ft_builtin_cd_change_directory(env, actual_pwd, path)))
    {
      return (EXIT_FAILURE);
    }
  }
 ...
 */
int  ft_builtin_cd(const char **tab_cmd, t_arr **env)
{
  int len_array;
  char *actual_pwd;
  ...
  if (len_array > 2)
  {
    ft_putstr("21sh: too many arguments\n");
    return (EXIT_FAILURE);
  }
  ...
  if (len_array > 1)
  {
    ft_builtin_cd_path(env, ft_strdup(tab_cmd[1]), actual_pwd);
  }
  else
  {
    ft_builtin_cd_path(env, HOME, actual_pwd);
  }
  return (EXIT_SUCCESS);
}
