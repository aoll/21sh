 ...
 */
int  ft_builtin_exec(int index_builtin, char **tab_cmd, t_arr **env)
{
  if (!index_builtin)
  {
    return (EXIT_FAILURE);
  ...
  {
    ft_builtin_env(tab_cmd, env);
  }
  else if (index_builtin == B_SETENV)
  {
    ft_builtin_setenv((const char **)tab_cmd, env);
  }
  else if (index_builtin == B_UNSETENV)
  {
    ft_builtin_unsetenv((const char **)tab_cmd, env);
  }
  else if (index_builtin == B_CD)
  {
    ft_builtin_cd((const char **)tab_cmd, env);
  }
  else if (index_builtin == B_ECHO)
  {
    ft_builtin_echo(tab_cmd, env);
  }
