...
int					ft_read_exit(
	t_list_arr *list_arr, t_cursor *cursor, struct termios *term);
...
int					ft_read_parse_ctrl(
	const char *buff, t_cursor *cursor, t_arr *arr);
int					ft_read_parse_arrow(
	const char *buff, t_cursor *cursor, t_list_arr *list_arr);
int					ft_read_parse_home_end(
	const char *buff, t_cursor *cursor, t_arr *arr);
int					ft_read_parse_ctrl_arrow(
	const char *buff, t_cursor *cursor, t_arr *arr);
...
int					ft_cursor_add_char(
	t_cursor *cursor, t_arr *arr, char *buff);
int					ft_nb_char_between_two_chariot(t_arr *arr, int index);
int					ft_arr_index_line_start_showed(
	t_cursor *cursor, t_arr *arr, int line_off);
int					ft_index_line_start_showed(t_cursor *cursor, t_arr *arr);
int					ft_cursor_restore_index(
	t_cursor *cursor, t_arr *arr, int index);
...
int					ft_cursor_paste(
	t_cursor *cursor, t_arr *arr, t_arr *copy_line);
...
int					ft_fork_free_st_fork_and_arr_fd(
	t_fork *st_fork, t_arr_fd *arr_fd);
int					ft_fork_init_tab_cmd(t_fork *st_fork, char *cmd);
int					ft_fork_init_env_copy(
	t_fork *st_fork, t_arr *env, t_tab_tube *array_tube);
int					ft_fork_set_env_copy(
	t_fork *st_fork, t_arr *env, t_tab_tube *array_tube);
int					ft_fork(
	char **cmd, t_tube *tab_tube, t_arr **env, int nb_pipe);
int					ft_fork_son_exec(
	char *path_cmd, char **tab_cmd, char **envp);
...
int					ft_cursor_clear_down(t_cursor *cursor);
int					ft_arr_add_char(
	t_cursor *cursor, t_arr *arr, char c, int is_tab);
int					ft_arr_index_line_start_showed(
	t_cursor *cursor, t_arr *arr, int line_off);
...
int					ft_cursor_left_same_line(
	t_cursor *cursor, unsigned char *s_line);
int					ft_nb_char_between_two_chariot(t_arr *arr, int index);
...
int					ft_fork_list_fd(
	char **command, t_arr_fd *arr_fd, char **error_ptr);
...
int					ft_fork_list_fd_tmp(
	char **cmd, t_arr_fd *arr_fd, char **error_ptr);
int					ft_fork_write_list_fd(t_arr *arr, char *buff, int len);
int					ft_fork_list_d_end_word(
	char **command, t_arr *arr_d_end_word);
...
char				*ft_fork_init_path(
	t_arr *env_copy, char **tab_cmd, int *err);
...
int					ft_fork_write_fd_stdout(
	...
	t_arr_fd *arr_fd, t_fork *st_fork);
...
int					ft_fork_check_is_builtin_exit(
	t_fork *st_fork, t_arr_fd *arr_fd);
int					ft_fork_pid_son(
...
char				*ft_builtin_cd_absolute_path(
...
int					ft_builtin_env_set_var_cmd(
	char **cmd, t_arr *env, bool *is_env_prev);
